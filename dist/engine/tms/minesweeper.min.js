...
lang.leave_game_confirm,{cancel:function(){},confirm:b,confirmButtonClass:app_dialog.options.buttons.BLUE,icon:"fas fa-home",size:app_dialog.options.size.SMALL})});this._dom.resetbutton.on("click",function(){var b=function(){a._reset=!0;app_tms.new()};app_sound.play(app_sound.sound.BUTTON);0===a._game_status.played||a._gameover?b():app_dialog.confirm(lang.reset_game_title,lang.reset_game_confirm,{cancel:function(){},confirm:b,confirmButtonClass:app_dialog.options.buttons.DANGER,icon:"fas fa-exclamation-triangle",
size:app_dialog.options.size.SMALL})});a._update_counters();b&&a._load_score(!0);a._check_size.opened=!1;a._check_size.enabled&&(app_dom.window.on("resize.checksize",a._check_window_size),a._check_window_size());this._dom.viewer.show();a._scoreboard_setup();a._reset=!1};this._check_window_size=function(){var b=app_dom.window.outerWidth(),c=app_dom.window.outerHeight();b>=a._check_size.width&&c>=a._check_size.height?(a._check_size.opened&&app_dialog.close_last(),a._check_size.opened=!1):a._check_size.opened||
(a._check_size.opened=!0,app_dialog.confirm(lang.check_window_size_title,lang.check_window_size_info.format(round_number(b),round_number(c),round_number(a._check_size.width),round_number(a._check_size.height)),{cancelText:null,confirm:function(){a._check_window_size()},confirmButtonClass:app_dialog.options.buttons.DANGER,confirmText:lang.answer_ok,icon:"fas fa-desktop",size:app_dialog.options.size.NORMAL}))};this._load_score=function(b){b&&a._scoreboard_setup();setTimeout(function(){a._get_score()},
1E3)};this._update_counters=function(){var b=round_number(100*a._game_status.played/a._game_status.total,1);isNaN(b)&&(b=0);a._dom.facecount.html("{0}/{1} ({2}%)".format(a._game_status.played,a._game_status.total,b));a._dom.flagcount.html(a._game_status.flags);a._dom.minecount.html(a._game_status.mines);a._dom.questioncount.html(a._game_status.questions)};this._request_username=function(){if(a._is_playing){var b=generateID(),c=sessionCookie.username;is_null_undf(c)&&(c="");app_dialog.form(lang.game_won_title,
'{2}.<br><br><form action="" class="formName"><div class="form-group"><label for="{0}">{1}:</label><input type="text" class="form-control" id="{0}" minlength="4" maxlength="20" value="{4}" required aria-required="true" placeholder="{5}" aria-placeholder="{5}" {3}></div></form>'.format(b,lang.game_won_name,lang.game_won_content.format(round_number(a._user.time,10>a._user.time?3:2)),""===c?"autofocus":"",c,lang.game_won_placeholder),function(){var c=a._sanitize_text($("#"+b).val());4<=c.length&&20>=
c.length&&(sessionCookie.username=c,update_session_cookie(),a._submit_score(c))},a._new_game_after_win,{cancelText:lang.dialog_form_cancel,icon:"fas fa-trophy",submitText:lang.dialog_form_send})}};this._sanitize_text=function(a){a=a.toString();a=a.replace(/[^a-z0-9\u00e1\u00e9\u00ed\u00f3\u00fa\u00f1\u00fc .,_-]/gim,"");return a.trim()};this._submit_score=function(b){app_library_manager.import_async_library(app_library_manager.lib.DBIP,function(){a._user.name=b;dbip.getVisitorInfo().then(function(b){a._user.location=
...
return}NotificationJS.error(lang.score_error_submit)}catch(f){app_console.exception(f)}finally{}app_console.exception(b)});b.fail(function(a,b,f){NotificationJS.error(lang.score_error_submit)});a._new_game_after_win()};this._new_game_after_win=function(){a._is_playing&&setTimeout(function(){app_dialog.confirm(lang.game_won_title,lang.start_new_game,{cancel:function(){},confirm:function(){a._reset=!0;app_tms.new()},backgroundDismiss:"cancel",confirmButtonClass:app_dialog.options.buttons.SUCCESS,escapeCancelKey:!0,
icon:"fas fa-trophy",size:app_dialog.options.size.SMALL})},1E3)};this._new_game_after_lose=function(){a._is_playing&&setTimeout(function(){app_dialog.confirm(lang.game_over,lang.start_new_game,{cancel:function(){},confirm:function(){a._reset=!0;app_tms.new()},backgroundDismiss:"cancel",confirmButtonClass:app_dialog.options.buttons.DANGER,escapeCancelKey:!0,icon:"fas fa-bomb",size:app_dialog.options.size.SMALL})},1E3)};this._get_score=function(){var b=$.ajax({crossOrigin:cfg_ajax_cors,data:"m=get&id={0}&g={1}".format(a._generator.id,
a._generator.type_id),timeout:cfg_href_ajax_timeout,type:"get",url:cfg_href_score});b.done(function(b){try{var c=JSON.parse(b);if(-1===Object.keys(c).indexOf("error")){a._write_scores(c);return}}catch(f){app_console.exception(f)}finally{}NotificationJS.error(lang.score_error_get);app_console.error(b);a._write_scores(null)});b.fail(function(b,d,f){NotificationJS.error(lang.score_error_get);a._write_scores(null)})};this._write_scores=function(b){var c=md5(b);if(c!==a._last_downloaded_score){a._last_downloaded_score=
c;a._dom.scoreboard_content.empty();if(not_null_undf(b)){for(var d=c=0;d<b.length;d+=1)a._write_user_scoreboard(b[d].user,d+1,b[d].country,b[d].date,b[d].time)&&(c+=1);0===c&&a._dom.scoreboard_content.html('<div class="game-scoreboard-empty">{0} <i class="fas fa-smile-wink"></i></div>'.format(lang.scoreboard_empty,a._get_scoreboard_height()))}else a._dom.scoreboard_content.html('<div class="game-scoreboard-empty"><i class="fas fa-exclamation-triangle"></i> {0}</div>'.format(lang.server_error));a._center_scoreboard()}};
